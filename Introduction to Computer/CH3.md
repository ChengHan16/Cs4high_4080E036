...
```
```
● 近場無線通訊（near ﬁeld communications，NFC）使用近距離（close-range）無線電波訊號，在兩個有開啟 NFC 功能的裝置之間傳輸資料。
  電腦和裝置只要具備適當的 Wi-Fi 功能，即可經由無線電波與其他 Wi-Fi 電腦或裝置進行通訊。
```
# 3-12　保護硬體
```
...
● 伺服器、超級電腦、POS 終端機、自動櫃員機（ATM）、自助式 kiosk 機器、電子書閱讀裝置、穿戴式裝置、遊戲裝置、嵌入式電腦和雲端運算
● 各式連接埠與接頭
● 保護硬體的方式
● 如何預防因為使用科技而造成的健康問題
```
